...
If a method call fails to execute properly&mdash;because of a machine crash or a
network partition, for example&mdash;it returns an error with an embedded
`weaver.SystemError`. Here's an illustrative example:
...
`weaver.SystemError`, it does *not* mean that the method never executed. The
method may have executed partially or fully. Thus, you must be careful retrying
method calls that result in a `weaver.SystemError`. Ensuring that all methods
are either read-only or idempotent is one way to ensure safe retries, for
example. Service Weaver does not automatically retry method calls that fail.
...
application code as `weaver.SystemError`, which can be handled as described in
