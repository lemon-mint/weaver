...
// SystemError indicates that a component method call failed to execute
// properly. This can happen, for example, because of a failed machine or a
// network partition. Here's an illustrative example:
//
//	// Call the foo.Foo method.
//	err := foo.Foo(ctx)
//	if errors.Is(err, weaver.SystemError) {
...
//	}
//
// Note that if a method call returns an error with an embedded SystemError, it
// does NOT mean that the method never executed. The method may have executed
// partially or fully. Thus, you must be careful retrying method calls that
// result in a SystemError. Ensuring that all methods are either read-only or
// idempotent is one way to ensure safe retries, for example.
var SystemError = errors.New("Service Weaver system error")
